...
const SearchDialog = ({ open, onOpenChange }: SearchDialogProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col rounded-[1.75rem]">
        <DialogHeader>
          ...
          {searchTerm === "" ? (
            ...
            filteredCourses.map((course) => (
              <div
                ...
                className="flex gap-4 p-3 rounded-2xl hover:bg-mist cursor-pointer transition-colors border border-transparent hover:border-primary/15"
              >
                <img
                  src={course.image}
                  alt={course.title}
