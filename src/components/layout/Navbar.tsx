    ...
    <div className="fixed top-0 left-0 right-0 z-50 flex justify-center px-3 sm:px-4 pt-3">
      <motion.nav
        ...
        className={`w-full max-w-6xl rounded-2xl border backdrop-blur-2xl transition-all duration-500 ${
          scrolled
            ? "bg-white/90 shadow-elevated border-primary/10"
            : "bg-white/70 shadow-soft border-white/60"
        }`}
      >
        <div className="px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14 sm:h-16">
            ...
            <Link to="/" className="flex items-center space-x-2 sm:space-x-3 group">
              ...
              >
                <img
                  src={logo}
                  alt="FaxLab AI Logo"
                  className="h-10 w-10 sm:h-11 sm:w-11 rounded-xl object-cover shadow-soft"
                />
              </motion.div>
              <motion.div whileHover={{ x: 2 }} className="flex flex-col">
                <span className="text-lg sm:text-xl font-extrabold text-foreground tracking-tight leading-none">
                  FaxLab <span className="text-primary">AI</span>
                </span>
                <span className="text-[9px] sm:text-[10px] text-muted-foreground tracking-wider uppercase">
                  Future. AI. X-factor
                </span>
              </motion.div>
            </Link>
            ...
            <div className="hidden md:flex items-center space-x-2 lg:space-x-3">
              <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
                <Button
                  variant="ghost"
                  size="icon"
                  className="text-foreground/60 hover:text-primary hover:bg-primary/10 rounded-full"
                ...
                <div className="flex items-center space-x-2">
                  <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                    <Button
                      variant="ghost"
                      size="sm"
                      className="text-foreground/70 hover:text-primary hover:bg-primary/5 text-sm"
            ...
            <motion.button
              ...
              className="md:hidden text-foreground hover:text-primary p-2 rounded-xl hover:bg-primary/10 transition-colors"
            >
          ...
          <AnimatePresence>
            {isOpen && (
              ...
              >
                <div className="py-3 space-y-1 border-t border-primary/10">
                  {userNavItems.map((item, i) => (
                    <motion.div
                      key={item.name}
                      custom={i}
                  ...
                  <motion.div
                    ...
                    initial="hidden"
                    animate="visible"
                    className="flex items-center gap-2 pt-3 px-1 border-t border-primary/10 mt-2"
                  >
                    <Button
                      variant="ghost"
                      size="sm"
                      className="text-foreground/70 hover:text-primary hover:bg-primary/10 rounded-xl"
                      onClick={() => setSearchOpen(true)}
                    >
                    ...
                    {user ? (
                      <>
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          className="justify-start rounded-xl text-foreground/70 hover:bg-primary/10"
                          onClick={() => {
                            navigate("/profile");
                            setIsOpen(false);
                          }}
