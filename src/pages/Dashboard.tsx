...
const statCards = [
  { key: "enrolled", title: "Enrolled Courses", icon: BookOpen, bg: "bg-primary/10", iconColor: "text-primary" },
  { key: "completed", title: "Completed", icon: CheckCircle2, bg: "bg-gold-light", iconColor: "text-gold-foreground" },
  { key: "certificates", title: "Certificates", icon: Award, bg: "bg-coral-light", iconColor: "text-coral-foreground" },
  { key: "hours", title: "Learning Hours", icon: Clock, bg: "bg-sky-light", iconColor: "text-sky-foreground" },
];
...
const Dashboard = () => {
  ...
  const statValues: Record<string, { value: number; sub: string }> = {
    enrolled: { value: totalEnrolled, sub: "Total courses enrolled" },
    completed: { value: completedCourses, sub: "Courses completed" },
    certificates: { value: totalCertificates, sub: "Certificates earned" },
    hours: { value: totalHours, sub: "Hours spent learning" },
  };

  // Data for pie chart
  const chartData = [
    { name: "Completed", value: completedCourses, color: "hsl(var(--gold))" },
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Hero Section */}
      <section className="pt-32 pb-14 md:pt-40 md:pb-16 bg-mist">
        ...
        <div className="container mx-auto px-4 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {statCards.map((card, i) => (
              ...
              >
                <Card className="border-0 shadow-soft rounded-[1.75rem] hover-lift">
                  ...
                  <CardContent>
                    ...
                    <p className="text-xs text-muted-foreground">{statValues[card.key].sub}</p>
                  </CardContent>
                </Card>
              </motion.div>
            ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            >
              <Card className="border-0 shadow-soft rounded-[1.75rem] h-full">
                <CardHeader>
                    ...
                    <div className="space-y-4">
                      {enrollments.map((enrollment: any) => {
                        ...
                        return (
                          <div
                            key={enrollment.id}
                            className="flex items-center gap-4 p-4 rounded-2xl bg-mist hover:bg-primary/5 smooth-transition"
                          >
                            <img
                            ...
                            <div className="flex-1 space-y-2">
                              <div className="flex items-center justify-between gap-2">
                                ...
                                {enrollment.completed && (
                                  <Badge className="bg-gold text-gold-foreground rounded-full shrink-0">
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {certificates.map((cert: any) => (
                      <div
                        key={cert.id}
                        className="p-6 rounded-[1.5rem] bg-gradient-to-br from-gold-light to-primary/10 space-y-3"
                      >
